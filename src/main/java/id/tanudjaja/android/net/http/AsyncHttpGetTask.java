...
public class AsyncHttpGetTask extends AsyncHttpTask {
    private final static String TAG = "AsyncHttpGetTask";

    /**
     * The constructor of AsyncHttpGetTask. This constructor calls the constructor of JAsyncHttpTask and passes "GET" as the method.
     *
     * @param aUrl      The target URL.
     * @param aListener The listener of a http-get operation.
     */
    public AsyncHttpGetTask(HttpGetRequest aHttpRequest, AsyncHttpTaskListener aListener) {
    ...
    protected HttpResponse doInBackground(Void... params) {
        ...
        Log.d(TAG, "checking cancellation before preparing http get with url: " + mHttpRequest.getUrl());
        if (isCancelled() == true)
		{
	...
	if(responseCode>=300 && responseCode<=300)
	{
        ...
        if (isGzip == true)
		{
			...
			{
                Log.d(TAG, "gzip IOException raised: " + e.getMessage());
		...
		else
		{
			...
			{
                Log.d(TAG, "IOException raised: " + e.getMessage());
                return new HttpResponse(errno.EIO, new byte[]{0}, "");
            }
        }
